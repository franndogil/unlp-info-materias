...
>>>>>>> 087c9198c145917bf40fb345dffdd630c4f818b1
	p.dia:=random(31)+1;
	p.mes:=random(12)+1;
	p.cantd:=random(90)+1;
end;
...
<<<<<<< HEAD
procedure Recorrer_lista(l:lista; nums:integer ;var cant:integer);
begin
	while(l<>nil)do begin
		if(nums=l^.dato.nums)then
			cant:=cant+1;
		l:=l^.sig;
	end;
end;

function Cant_pres_2(a:arbol2; nums:integer):integer;		//PUNTO E
var
	cant:integer;
begin
	cant:=0;
	if(a=nil)then
		...
		Recorrer_lista(a^.dato.todos, nums ,cant);
		Cant_pres_2:=cant+Cant_pres_2(a^.HI, nums)+Cant_pres_2(a^.HD, nums);
	end;
end;

procedure Insertar_ordenado(var l:listamf; a:lmodf);
var
	nue, ant, act:listamf;
begin
	new(nue);
	...
	while(act<>nil)and(a.isbn<act^.dato.isbn)do begin
		ant:=act;
		act:=act^.sig;
	end;
...
procedure Modulo_f(a:arbol1; var l:listamf);
var
	aux:lmodf;
begin
	if(a<>nil)then begin
		aux.isbn:=a^.dato.isbn;
		aux.cant:=1;
		writeln('pasa 2');
		if(l=nil)then
			Insertar_ordenado(l, aux);
		while(l<>nil)and(a^.dato.isbn<>l^.dato.isbn)do begin
			if(a^.dato.isbn=l^.dato.isbn)then begin
...
procedure Impresion_de_estructura3(l:listamf);
begin
	while(l<>nil)do begin
...
<<<<<<< HEAD
	...
	writeln('Ingrese un numero de socio: ');
	read(numsi);
	writeln('Cantidad de prestamos realizados a dicho socio: ', Cant_pres_2(a2, numsi));		//PUNTO E
	
	Modulo_f(a1, lf);
	Impresion_de_estructura3(lf);
=======
>>>>>>> 087c9198c145917bf40fb345dffdd630c4f818b1
end.
